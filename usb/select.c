...
for(i = 0 ; i < 125 * 200; i++) {
	...
	fd_set fds,fds_read;  //select 查询集合
	int timeout_ms = 1000;
	...
	FD_SET(usb_fd_send,&fds_read); //添加描述符
	maxfdp=usb_fd_send+1;    //描述符最大值加1  
	switch(select(maxfdp,&fds_read,&fds,NULL,&timeout)) { //select使用
	...
	case 0:  //超时，退出程序
		...
		return 0;
	default:
		if(FD_ISSET(usb_fd_send,&fds_read)) { //测试文件是否可读
		printf("read num = %d\n",read(usb_fd_send,REC_BUF,20));   //返回接收数据个数
		}
		else if(FD_ISSET(usb_fd_send,&fds)) { //测试文件是否可写
		write_all = write_all + write(usb_fd_send,SEND_BUF,BUFF_LENGTH);   //返回发送数据个数
		}
	}// end switch
